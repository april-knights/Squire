...
@endif
<h1>Edit Order</h1>
<form method="POST" id="edit" action="/orders/{{ $order->pkey }}/edit">
    @csrf
    <div class="row">
        <div class="col-md">
            <div class="form-group">
                ...
                <input class="form-control" id="title" name="title" type="text"
                    value="{{ old('title', $order->title) }}" maxlength="255">
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
                ...
                <select class="form-control" id="level" name="level" onchange="toggleBattalion(this.value)">
                    ...
                    <option value="{{ $value }}"
                        {{ old('level', $order->level) == $value ? 'selected' : '' }}>
                        {{ $label }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="fkeybattalion" name="fkeybattalion">
                    ...
                    <option value="{{ $battalion->pkey }}"
                        {{ old('fkeybattalion', $order->fkeybattalion) == $battalion->pkey ? 'selected' : '' }}>
                        {{ $battalion->name }}
                    </option>
                    @endforeach
                </select>
    ...
    <div class="row">
        <div class="col">
            <div class="form-group">
                ...
                <textarea class="form-control" id="body" name="body">{{ old('body', $order->body) }}</textarea>
            </div>
        </div>
    </div>
    <div class="row">
