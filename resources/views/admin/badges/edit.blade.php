@extends('layouts.app')
@section('title', 'Admin — Edit ' . $badge->bdg_title)
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="/admin/badges/{{ $badge->pkey }}">{{ $badge->bdg_title }}</a></li>
        <li class="breadcrumb-item active">Edit</li>
    </ol>
</nav>

<h2 class="mb-3">Edit Badge — {{ $badge->bdg_title }}</h2>

<form method="POST" action="/admin/badges/{{ $badge->pkey }}/edit" enctype="multipart/form-data">
    @csrf
    @method('PUT')
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="text" class="form-control @error('bdg_title') is-invalid @enderror"
                           id="bdg_title" name="bdg_title"
                           value="{{ old('bdg_title', $badge->bdg_title) }}" required maxlength="255">
                ...
                <div class="col-sm-4">
                    <select class="form-control @error('typcd') is-invalid @enderror" id="typcd" name="typcd" required>
                        @foreach($typecds as $t)
                            <option value="{{ $t }}" {{ old('typcd', $badge->typcd) === $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <textarea class="form-control @error('bdgdesc') is-invalid @enderror"
                              id="bdgdesc" name="bdgdesc" rows="3" maxlength="500">{{ old('bdgdesc', $badge->bdgdesc) }}</textarea>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    <input type="text" class="form-control @error('roleid') is-invalid @enderror"
                           ...
                           value="{{ old('roleid', $badge->roleid) }}" maxlength="25">
            ...
            <div class="row">
                <div class="col-md-2">
                    <div class="img-preview-wrap mb-2">
                        <img id="imgPreview"
                             src="{{ asset($badge->imgurl ?? 'static/img/badges/NoArtYet.jpg') }}"
                             alt="Preview">
                    </div>
                    <div class="small text-muted text-center" id="imgPreviewLabel">
                        {{ $badge->imgurl ? basename($badge->imgurl) : 'Placeholder' }}
                    </div>
                    @if($badge->imgurl)
                    <div class="form-check mt-2">
                        <input type="checkbox" class="form-check-input" id="clear_image" name="clear_image" value="1">
                        <label class="form-check-label small text-danger" for="clear_image">Clear image</label>
                    </div>
                    @endif
                </div>
                ...
                <div class="col-md-10">
                    <ul class="nav nav-tabs mb-3" id="imgTabs" style="border-color: #8b3a3a;">
                        <li class="nav-item">
                            <a class="nav-link active" href="#" data-tab="pick"
                               style="color:#efefef; border-color:#8b3a3a #8b3a3a transparent;">Pick Existing</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#" data-tab="upload"
                               style="color:#c9a0a0;">Upload New</a>
                        </li>
                    </ul>

                    <div id="panel-pick">
                        <input type="hidden" name="imgurl" id="imgurl" value="{{ old('imgurl', $badge->imgurl) }}">
                        @foreach($imgFiles as $dir => $files)
                            <div class="img-picker-dir-title">{{ $dir }}</div>
                            <div class="img-picker-grid">
                                @foreach($files as $path)
                                <div class="img-picker-item {{ old('imgurl', $badge->imgurl) === $path ? 'selected' : '' }}"
                                     data-path="{{ $path }}">
                    ...
                    <div id="panel-upload" style="display:none;">
                        <div class="form-group row">
                            <label class="col-sm-3 col-form-label" for="img_subdir_select">Subdirectory</label>
                            <div class="col-sm-5">
    ...
    <div class="form-group row">
        <div class="col-sm-9 offset-sm-3">
            ...
            <a href="/admin/badges/{{ $badge->pkey }}" class="btn btn-outline-secondary ml-2">Cancel</a>
        </div>
    </div>
...
(function () {
    ...
    document.querySelectorAll('.img-picker-item').forEach(function (item) {
        item.addEventListener('click', function () {
            ...
            var clearCb = document.getElementById('clear_image');
