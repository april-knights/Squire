@extends('layouts.app')
@section('title', 'Admin — Edit ' . $skill->skillname)
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="/admin/skills/{{ $skill->pkey }}">{{ $skill->skillname }}</a></li>
        <li class="breadcrumb-item active">Edit</li>
    </ol>
</nav>

<h2 class="mb-3">Edit Skill — {{ $skill->skillname }}</h2>

<form method="POST" action="/admin/skills/{{ $skill->pkey }}/edit">
    @csrf
    @method('PUT')
                ...
                <div class="col-sm-5">
                    <select class="form-control @error('parentid') is-invalid @enderror" id="parentid" name="parentid">
                        ...
                        @foreach($parents as $p)
                            <option value="{{ $p->pkey }}"
                                {{ old('parentid', $skill->parentid) == $p->pkey ? 'selected' : '' }}>
                                {{ $p->skillname }}
                            </option>
                        @endforeach
                    </select>
                    <small class="form-text text-muted">
                        Changing a group to a child skill will move all its children to orphaned status.
                    </small>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="text" class="form-control @error('skillname') is-invalid @enderror"
                           ...
                           value="{{ old('skillname', $skill->skillname) }}" required maxlength="64">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="text" class="form-control @error('skilldescr') is-invalid @enderror"
                           id="skilldescr" name="skilldescr"
                           value="{{ old('skilldescr', $skill->skilldescr) }}" maxlength="255">
                ...
                <div class="col-sm-9 offset-sm-3">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="public" name="public" value="1"
                               {{ old('public', $skill->public) ? 'checked' : '' }}>
    ...
    <div class="form-group row">
        <div class="col-sm-9 offset-sm-3">
            ...
            <a href="/admin/skills/{{ $skill->pkey }}" class="btn btn-outline-secondary ml-2">Cancel</a>
        </div>
    </div>
