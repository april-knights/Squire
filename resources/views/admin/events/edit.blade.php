@extends('layouts.app')
@section('title', 'Admin — Edit ' . $event->title)
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="/admin/events/{{ $event->pkey }}">{{ $event->title }}</a></li>
        <li class="breadcrumb-item active">Edit</li>
    </ol>
</nav>

<h2 class="mb-3">Edit Event — {{ $event->title }}</h2>

@php
    $livedate = $event->livedate && $event->livedate !== '0000-00-00' ? $event->livedate : '';
    $enddate  = $event->enddate  && $event->enddate  !== '0000-00-00' ? $event->enddate  : '';
@endphp

<form method="POST" action="/admin/events/{{ $event->pkey }}/edit">
    @csrf
    @method('PUT')
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-5">
                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                           id="title" name="title"
                           value="{{ old('title', $event->title) }}" required maxlength="30">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <textarea class="form-control @error('eventdescr') is-invalid @enderror"
                              id="eventdescr" name="eventdescr" rows="3" maxlength="500">{{ old('eventdescr', $event->eventdescr) }}</textarea>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    <input type="text" class="form-control @error('organizer') is-invalid @enderror"
                           ...
                           value="{{ old('organizer', $event->organizer) }}" maxlength="30">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-3">
                    <input type="date" class="form-control @error('livedate') is-invalid @enderror"
                           id="livedate" name="livedate" value="{{ old('livedate', $livedate) }}">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-3">
                    <input type="date" class="form-control @error('enddate') is-invalid @enderror"
                           id="enddate" name="enddate" value="{{ old('enddate', $enddate) }}">
                    <small class="form-text text-muted">Leave blank if unknown.</small>
                ...
                <div class="col-sm-3">
                    <select class="form-control @error('eventtype') is-invalid @enderror" id="eventtype" name="eventtype" required>
                        @foreach($eventTypes as $t)
                            <option value="{{ $t }}" {{ old('eventtype', $event->eventtype) === $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-9 offset-sm-3">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="profileflg" name="profileflg" value="1"
                               {{ old('profileflg', $event->profileflg) ? 'checked' : '' }}>
    ...
    <div class="form-group row">
        <div class="col-sm-9 offset-sm-3">
            ...
            <a href="/admin/events/{{ $event->pkey }}" class="btn btn-outline-secondary ml-2">Cancel</a>
        </div>
    </div>
